...
let serverLogbooks = [];
let salesData = { lines: [], writeoffs: [], prices: {}, totals: { produced: 0, writtenOff: 0, sold: 0, revenue: 0 } };
let stockPriceSaveTimer = null;
...
function requiredRulesFor(product){
  ...
  const rules = {
    ...
    melk:[...common,{id:'m-date',label:'Datum productie'},{id:'m-start',label:'Tijdstip start'},{id:'m-liters',label:'Liters melk'},{id:'m-programma',label:'Programma pasteur'}],
...
function updateDates(){
  ...
  const yProcessDate = val('#y-verwerk-datum') || yDate;
  const yAftapDate = val('#y-aftap-date') || yProcessDate;
  const yHangopDate = val('#y-hangop-datum') || yProcessDate;
  $('#y-tht') && ($('#y-tht').value = addDaysLocal(yProcessDate,16));
  $('#y-aftap-tht') && ($('#y-aftap-tht').value = addDaysLocal(yAftapDate,16));
  $('#y-hangop-tht') && ($('#y-hangop-tht').value = addDaysLocal(yHangopDate,16));

  const kmProd = val('#km-prod-date');
  const kmProcess = val('#km-verwerk-datum');
  const kmFill = val('#km-date') || kmProcess;
  const kmAftapDate = val('#km-aftap-date') || kmProcess || kmProd;
  $('#km-tht') && ($('#km-tht').value = addDaysLocal(kmProcess || kmFill,14));
  $('#km-aftap-tht') && ($('#km-aftap-tht').value = addDaysLocal(kmAftapDate,16));
...
function updateSummary(){
  ...
  if(product === 'yoghurt'){
    ...
    if($('#y-opt-aftap')?.checked){
      out.push(sec('Aftap melk', [
        kv('Liters aftap', val('#y-aftap-liters')), kv('Datum aftap/schenken', val('#y-aftap-date')), kv('Tijdstip aftap', val('#y-aftap-tijd')),
  ...
  if(product === 'karnemelk'){
    ...
    if($('#km-opt-aftap')?.checked){
      out.push(sec('Aftap melk', [
        kv('Liters aftap', val('#km-aftap-liters')), kv('Datum aftap/schenken', val('#km-aftap-date')), kv('Tijdstip aftap', val('#km-aftap-tijd')),
  ...
  if(product === 'melk'){
    out.push(sec('Gepasteuriseerde melk', [
      kv('Datum productie', val('#m-date')), kv('Start', val('#m-start')), kv('Liters melk', val('#m-liters')),
      kv('Programma pasteur', val('#m-programma')), kv('Temperatuur bij afvullen (°C)', val('#m-temp')), kv('Tijdstip in koeling', val('#m-koeltijd')),
...
function buildState(){
  ...
  $$('input[id], select[id], textarea[id]').forEach(el => {
    if(el.closest('header') || el.closest('#logbook-panel') || el.closest('#stock-panel') || el.closest('.mobile-actionbar')) return;
...
function stockLineMatchesFilters(line){
  const start = val('#stock-start');
  const end = val('#stock-end');
  const product = val('#stock-product-filter');
  const search = val('#stock-search').toLowerCase().trim();
  const date = line.productionDate || '';
  const haystack = [line.product, line.flavor, line.size, line.tht, line.logbookTitle].join(' ').toLowerCase();
  return (!start || date >= start) &&
    (!end || date <= end) &&
    (!product || line.product === product) &&
    (!search || haystack.includes(search));
}
function stockVisibleLines(){
  return (salesData.lines || []).filter(stockLineMatchesFilters);
}
function stockTotals(lines){
  return lines.reduce((acc, line) => {
    acc.produced += Number(line.quantity) || 0;
    acc.writtenOff += Number(line.writtenOff) || 0;
    acc.sold += Number(line.sold) || 0;
    acc.revenue += Number(line.revenue) || 0;
    return acc;
  }, {produced:0, writtenOff:0, sold:0, revenue:0});
}
function renderStockProductFilter(){
  const sel = $('#stock-product-filter');
  if(!sel) return;
  const keep = sel.value;
  const products = [...new Set((salesData.lines || []).map(line => line.product).filter(Boolean))].sort((a,b) => a.localeCompare(b));
  sel.innerHTML = '<option value="">Alle producten</option>' + products.map(product => `<option value="${escapeHtml(product)}">${escapeHtml(product)}</option>`).join('');
  if(products.includes(keep)) sel.value = keep;
}
function renderStockTotals(lines){
  const totals = stockTotals(lines);
  const cards = $$('#stock-totals strong');
  if(cards[0]) cards[0].textContent = qtyText(totals.produced);
  if(cards[1]) cards[1].textContent = qtyText(totals.writtenOff);
  if(cards[2]) cards[2].textContent = qtyText(totals.sold);
  if(cards[3]) cards[3].textContent = euro(totals.revenue);
}
function stockPriceRows(){
  const rows = new Map();
  for(const line of (salesData.lines || [])){
    if(!rows.has(line.priceKey)){
      rows.set(line.priceKey, {
...
function renderStockPrices(){
  const list = $('#stock-prices');
  if(!list) return;
  const rows = stockPriceRows();
  if(!rows.length){
    list.innerHTML = '<p class="muted">Nog geen producten om prijzen voor in te vullen.</p>';
    return;
  }
  list.innerHTML = rows.map(row => `
    <label class="price-item">
      <span>
        <strong>${escapeHtml(row.product)} ${escapeHtml(row.flavor)}</strong>
        <span>${escapeHtml(row.size)}</span>
      </span>
      <input type="number" min="0" step="0.01" value="${moneyInputValue(row.unitPrice)}" data-price-key="${escapeHtml(row.priceKey)}" aria-label="Prijs voor ${escapeHtml(row.product)} ${escapeHtml(row.flavor)} ${escapeHtml(row.size)}"/>
    </label>
  `).join('');
}
function renderStockLines(){
  const body = $('#stock-lines');
  if(!body) return;
  const lines = stockVisibleLines();
  renderStockTotals(lines);
  if(!lines.length){
    body.innerHTML = '<tr><td colspan="10" class="muted">Geen regels gevonden.</td></tr>';
    return;
  }
  body.innerHTML = lines.map(line => `
    <tr>
      ...
      <td>${escapeHtml(line.tht || '-')}</td>
      <td class="num">${qtyText(line.quantity)}</td>
      <td class="num">${qtyText(line.writtenOff)}</td>
      <td class="num">${qtyText(line.sold)}</td>
      <td><input type="number" min="0" step="0.01" value="${moneyInputValue(line.unitPrice)}" data-price-key="${escapeHtml(line.priceKey)}" aria-label="Prijs per stuk"/></td>
      <td class="num">${euro(line.revenue)}</td>
      <td><button type="button" class="btn" data-stock-action="writeoff" data-line-id="${escapeHtml(line.lineId)}">Afboeken</button></td>
    </tr>
  `).join('');
}
function renderWriteoffs(){
  const list = $('#writeoff-list');
  if(!list) return;
  if(!salesData.writeoffs?.length){
    list.innerHTML = '<p class="muted">Nog geen afboekingen.</p>';
    return;
  }
  list.innerHTML = salesData.writeoffs.map(item => `
    <div class="writeoff-item">
      <p><strong>${escapeHtml(item.product)} ${escapeHtml(item.flavor)} ${escapeHtml(item.size)}</strong><br>
      <span class="muted">${escapeHtml(item.date || '')} - ${qtyText(item.quantity)} stuks - ${escapeHtml(item.reason || 'Afgeboekt')}${item.note ? ` - ${escapeHtml(item.note)}` : ''}</span></p>
      <button type="button" class="btn danger" data-stock-action="delete-writeoff" data-writeoff-id="${escapeHtml(item.id)}">Verwijderen</button>
    </div>
  `).join('');
}
...
async function loadSalesReport(){
  try{
    salesData = await apiJson('/api/sales');
    renderStock();
  }catch(err){
    const body = $('#stock-lines');
    if(body) body.innerHTML = `<tr><td colspan="10" class="muted">${escapeHtml(err.message)}</td></tr>`;
  }
}
...
async function saveStockPrices(){
  ...
  $$('input[data-price-key]').forEach(input => {
    const value = Number(String(input.value || '').replace(',', '.')) || 0;
    if(value) prices[input.dataset.priceKey] = value;
    else delete prices[input.dataset.priceKey];
  });
  salesData.prices = prices;
  await apiJson('/api/prices', {method:'PUT', body: JSON.stringify({prices})});
  await loadSalesReport();
}
function syncStockPriceInput(input){
  if(!input?.dataset?.priceKey) return;
  const key = input.dataset.priceKey;
  $$(`input[data-price-key="${CSS.escape(key)}"]`).forEach(other => {
    if(other !== input) other.value = input.value;
  });
}
function formatStockPriceInput(input){
  if(!input) return;
  input.value = moneyInputValue(input.value);
}
function scheduleStockPriceSave(input){
  syncStockPriceInput(input);
  window.clearTimeout(stockPriceSaveTimer);
  stockPriceSaveTimer = window.setTimeout(saveStockPrices, 500);
}
async function addWriteoff(lineId){
  const line = (salesData.lines || []).find(item => item.lineId === lineId);
  if(!line) return;
  const remaining = Math.max(0, Number(line.sold) || 0);
  if(!remaining){
    alert('Er is niets meer om af te boeken voor deze regel.');
    return;
  }
  const quantity = Number(String(prompt(`Aantal afboeken voor ${line.product} ${line.flavor} ${line.size}? Maximaal ${qtyText(remaining)}.`, '1') || '').replace(',', '.'));
  if(!quantity || quantity <= 0) return;
  if(quantity > remaining && !confirm('Je boekt meer af dan er nog als verkocht werd aangenomen. Toch doorgaan?')) return;
  const reason = String(prompt('Reden afboeken:', 'Weggegooid') || '').trim() || 'Afgeboekt';
  const note = String(prompt('Opmerking (optioneel):') || '').trim();
  await apiJson('/api/writeoffs', {
    method:'POST',
    body: JSON.stringify({lineId, quantity, reason, note})
  });
  await loadSalesReport();
}
async function deleteWriteoff(id){
  if(!confirm('Deze afboeking verwijderen?')) return;
  await apiJson(`/api/writeoffs/${encodeURIComponent(id)}`, {method:'DELETE'});
  await loadSalesReport();
}
async function saveCurrentLogbookToServer(){
  ...
  try{
    ...
    if(!$('#stock-panel')?.hidden) await loadSalesReport();
...
function fillDemo(){
  ...
  $('#y-aftap-liters').value='30'; $('#y-aftap-date').value=today; $('#y-aftap-tijd').value='10:30'; $('#y-aftap-temp').value='32'; $('#y-aftap-1l').value='10'; $('#y-aftap-500').value='5';
  ...
  $('#m-date').value=today; $('#m-start').value='08:00'; $('#m-liters').value='60'; $('#m-programma').value='63°C 30 min'; $('#m-temp').value='6'; $('#m-koeltijd').value='10:30'; $('#m-1l').value='30'; $('#m-500').value='20';
...
function init(){
  ...
  $('#btnShowStock').addEventListener('click', showStockPanel);
  ...
  $('#btnPanelShowStock').addEventListener('click', showStockPanel);
  ...
  $('#mobileShowStock').addEventListener('click', showStockPanel);
  $('#btnRefreshStock').addEventListener('click', loadSalesReport);
  $('#btnCloseStock').addEventListener('click', hideStockPanel);
  ['#stock-start', '#stock-end', '#stock-search'].forEach(selector => {
    $(selector)?.addEventListener('input', renderStockLines);
  });
  $('#stock-product-filter').addEventListener('change', renderStockLines);
  $('#stock-lines').addEventListener('click', e => {
    const btn = e.target.closest('button[data-stock-action="writeoff"]');
    if(btn) addWriteoff(btn.dataset.lineId);
  });
  $('#stock-lines').addEventListener('change', e => {
    if(e.target.matches('input[data-price-key]')){
      formatStockPriceInput(e.target);
      syncStockPriceInput(e.target);
      saveStockPrices();
    }
  });
  $('#stock-lines').addEventListener('input', e => {
    if(e.target.matches('input[data-price-key]')) scheduleStockPriceSave(e.target);
  });
  $('#stock-prices').addEventListener('change', e => {
    if(e.target.matches('input[data-price-key]')){
      formatStockPriceInput(e.target);
      syncStockPriceInput(e.target);
      saveStockPrices();
  ...
  $('#stock-prices').addEventListener('input', e => {
    if(e.target.matches('input[data-price-key]')) scheduleStockPriceSave(e.target);
  });
  $('#writeoff-list').addEventListener('click', e => {
    const btn = e.target.closest('button[data-stock-action="delete-writeoff"]');
    if(btn) deleteWriteoff(btn.dataset.writeoffId);
  });
  ...
  document.addEventListener('input', e => {
    if(e.target.closest('#stock-panel')) return;
    if(e.target.matches('input,select,textarea')){
      updateSummary();
      if(!e.target.closest('header') && !e.target.closest('#logbook-panel') && !e.target.closest('.server-toolbar')) markDirty();
    }
  });
